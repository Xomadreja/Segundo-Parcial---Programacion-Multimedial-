...
if(!empty($_POST["btnLogin"])){
    ...
    $sqlKardex=$conexion->query("select * from persona xp where xp.nombre='$user' and xp.password='$password' and xp.rol = 'Kardex'");
    $sqlCaja=$conexion->query("select * from persona xp where xp.nombre='$user' and xp.password='$password' and xp.rol = 'Caja'");
    if ($datos=$sqlEstudiante->fetch_object()) {
        $_SESSION['id']=$datos->id;
        $_SESSION['nombre']=$datos->nombre;
        $rol=$datos->rol;
    }
    else if ($datos=$sqlKardex->fetch_object()) {
        $_SESSION['id']=$datos->id;
        $_SESSION['nombre']=$datos->nombre;
        $rol=$datos->rol;
    } 
    else if ($datos=$sqlCaja->fetch_object()) {
        $_SESSION['id']=$datos->id;
        $_SESSION['nombre']=$datos->nombre;
        $rol=$datos->rol;
    }
